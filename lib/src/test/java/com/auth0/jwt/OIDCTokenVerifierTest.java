...
public class OIDCTokenVerifierTest {
    ...
    public void failsToCreateOptionsWhenIssuerIsNull() {
        ...
        OIDCTokenVerifier.init(null, "audience", signatureVerifier);
    }

    @Test
    public void failsToCreateOptionsWhenAudienceIsNull() {
        exception.expect(NullPointerException.class);
        OIDCTokenVerifier.init("issuer", null, signatureVerifier);
    }

    @Test
    public void failsToCreateOptionsWhenVerifierIsNull() {
        exception.expect(NullPointerException.class);
        OIDCTokenVerifier.init("issuer", "audience", null);
    }
    ...
    public void failsWhenIDTokenMissing() {
        ...
        OIDCTokenVerifier.init("issuer", "audience", signatureVerifier)
                .build()
                .verify((String) null);
    }

    @Test
    public void failsWhenIDDecodedJWTMissing() {
        exception.expect(JWTVerificationException.class);
        exception.expectMessage("ID token is required but missing");

        OIDCTokenVerifier.init("issuer", "audience", signatureVerifier)
                .build()
                .verify((DecodedJWT) null);
    }
    ...
    public void failsWhenTokenCannotBeDecoded() {
        ...
        OIDCTokenVerifier.init(DOMAIN, AUDIENCE, signatureVerifier)
                .build()
                .verify(token);
    }
    ...
    public void failsWhenSignatureIsInvalid() {
        ...
        OIDCTokenVerifier.init(DOMAIN, AUDIENCE, verifier)
                .build()
                .verify(token);
    }
    ...
    public void failsWhenIssuerMissing() {
        ...
        configureOptions(token).build().verify(token);
    }
    ...
    public void failsWhenExpClaimInvalidOutsideDefaultLeeway() {
        ...
        configureOptions(token).withClock(clock).build().verify(token);
    }
    ...
    public void failsWhenExpClaimInvalidOutsideCustomLeeway() {
        ...
        configureOptions(token)
                .withClock(clock)
                .withClockSkew(leeway)
                .build()
                .verify(token);
    }
    ...
    public void succeedsWithValidTokenUsingDefaultClock() {
        ...
        OIDCTokenVerifier.init("https://" + DOMAIN + "/", AUDIENCE, verifier)
                .withNonce("nonce")
                .build()
                .verify(token);
    }
    ...
    @Test
    public void succeedsWithValidDecodedJWTUsingDefaultClock() {
        String token = JWT.create()
                .withSubject("auth0|sdk458fks")
                .withAudience(AUDIENCE)
                .withIssuedAt(getYesterday())
    ...
    public void succeedsWithValidTokenUsingDefaultClockAndHttpDomain() {
        ...
        OIDCTokenVerifier.init("http://" + DOMAIN + "/", AUDIENCE, verifier)
                .withNonce("nonce")
                .build()
                .verify(token);
    }
    ...
    private OIDCTokenVerifier.Builder configureOptions(String token) {
        DecodedJWT decodedJWT = JWT.decode(token);
        SignatureVerifier verifier = mock(SignatureVerifier.class);
        when(verifier.verifySignature(token)).thenReturn(decodedJWT);

        return OIDCTokenVerifier.init("https://" + DOMAIN + "/", AUDIENCE, verifier)
